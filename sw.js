...
async function backgroundCheck() {
  try {
    ...
    // Poll the device IP — the firmware requires HTTP Basic Auth on this
    // endpoint (checkWebAuth()), so the same admin password stored by the
    // main app must be sent here too, or every background check gets a 401.
    const authHeaders = cfg.devicePass
      ? { 'Authorization': 'Basic ' + btoa('admin:' + cfg.devicePass) }
      : {};
